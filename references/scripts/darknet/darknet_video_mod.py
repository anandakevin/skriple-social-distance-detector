...
def cvDrawBoxes(detections, img):
    ...
    for detectionOld in detections:
        # print("detectionOld", detectionOld)
        # detection[1] = float(detection[1])
        # print("detectionOld[1]", detectionOld[1])
        tempvar = float(detectionOld[1])
        # print("tempvar: ", tempvar)
        detection = (detectionOld[0], tempvar, detectionOld[2])
...
def YOLO():
    ...
    weightPath = "./Models/yolov4.weights"                                 # Path to weights
    ...
    cap = cv2.VideoCapture("./Input/video3.mp4")                             # Local Stored video detection - Set input video
    frame_width = int(cap.get(3))                                   # Returns the width and height of capture video
    frame_height = int(cap.get(4))
    # Set out for video writer
    out = cv2.VideoWriter(                                          # Set the Output path for video writer
        "./Output/output.avi", cv2.VideoWriter_fourcc(*"MJPG"), 10.0,
